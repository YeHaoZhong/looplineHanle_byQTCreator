...
StepLogger* StepLogger::instance = nullptr;
...
StepLogger::StepLogger() {
    ...
    std::filesystem::path dayPath = rootPath / yyyy.str() / mm.str() / dd.str() / "step";
...
StepLogger& StepLogger::getInstance() {
    static std::once_flag s_onceFlag;
    std::call_once(s_onceFlag, []() {
        instance = new StepLogger();
    });
    return *instance;
}
...
void StepLogger::Log(const std::string& message) {
...
void StepLogger::rotateLogFileIfNeeded() {
    // 获取当前本地时间信息
    auto tm_time = getLocalTm();
...
void StepLogger::ensureLogDirectory() {
...
std::string StepLogger::getLogFileName() {
    // 生成 log_YYYY-MM-DD_HH.txt，使用 current time
    auto tm_time = getLocalTm();
    char fileName[64];
    strftime(fileName, sizeof(fileName), "StepLog_%Y-%m-%d_%H.txt", &tm_time);
    return std::string(fileName);
}

std::string StepLogger::getCurrentDate() {
    // 返回 "YYYY-MM-DD"
    auto tm_time = getLocalTm();
    char buffer[20];
...
std::string StepLogger::getCurrentTime() {
    auto now = std::chrono::system_clock::now();
    auto now_time_t = std::chrono::system_clock::to_time_t(now);
    std::tm tm_time;
#ifdef _WIN32
