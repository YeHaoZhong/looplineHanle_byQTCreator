...
std::tuple<std::string, std::string, int, int> splitUdpMessage(const std::string& msg, int num)	//单号,重量,供包台号, 小车号
{
    ...
    try
    {
        if (parts.size() >= 4)
        {
            ...
            int car_id = std::stoi(parts[3]);
            return std::make_tuple(code, weight, supply_id, car_id);
        }
    }
